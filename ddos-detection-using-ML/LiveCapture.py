...
def main(featureAlgorithm, classification,a):
    ...
    while (data == 0):
        ...
        if(featureAlgorithm == 1 and classification == 1):
            ...
            predictions = LiveCaptureHelper.RFE_NB(window_dataframe)
            a.extend(predictions)
        elif(featureAlgorithm == 3 and classification == 2):
            predictions = LiveCaptureHelper.RFE_SVM(window_dataframe)
            a.extend(predictions)
        elif(featureAlgorithm == 3 and classification == 3):
            predictions = LiveCaptureHelper.RFE_DT(window_dataframe)
            a.extend(predictions)
        elif(featureAlgorithm == 3 and classification == 4):
            predictions = LiveCaptureHelper.RFE_RF(window_dataframe)
            a.extend(predictions)
            
        elif(featureAlgorithm == 4 and classification == 1):
            predictions = LiveCaptureHelper.WRFS_NB(window_dataframe)
            a.extend(predictions)
        elif(featureAlgorithm == 4 and classification == 2):
            predictions = LiveCaptureHelper.WRFS_SVM(window_dataframe)
            a.extend(predictions)
        elif(featureAlgorithm == 4 and classification == 3):
            predictions = LiveCaptureHelper.WRFS_DT(window_dataframe)
            a.extend(predictions)
        elif(featureAlgorithm == 4 and classification == 4):
            predictions = LiveCaptureHelper.WRFS_RF(window_dataframe)
            a.extend(predictions)
